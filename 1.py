...
# ===================== 核心配置（必填）=====================
DEEPSEEK_API_KEY = "sk-a0d31013de044042ad66c4baec00df48"  # 替换为自己的API Key
DEEPSEEK_BASE_URL = "https://api.deepseek.com/v1/chat/completions"

# 极简核心提示词（删除冗余规则/示例，仅保留关键要求）
SYSTEM_PROMPT = """
...
def clean_ai_output(raw_output: str) -> str:
    """
    清洗AI输出的JSON字符串，去除多余空白、注释、特殊字符，提取纯JSON
    """
    # 1. 去除首尾空白
    ...
    # 3. 提取JSON数组部分（匹配[]包裹的内容）
    json_match = re.search(r'\[.*\]', cleaned, flags=re.DOTALL)
    if json_match:
        cleaned = json_match.group(0)
    # 4. 替换多余的换行/缩进为空格（避免解析中断）
    cleaned = re.sub(r'\s+', ' ', cleaned)
    logging.debug(f"清洗后的JSON：{cleaned[:200]}...")  # 打印前200字符调试
    return cleaned

# ===================== 核心函数（修复所有问题）=====================
def call_deepseek_json_mode(english_article: str) -> list:
    """
    调用DeepSeek API（适配JSON Mode）处理英语文章，返回标准化JSON数组
    """
    # 构建API请求体（按官方要求拆分system/user角色）
    request_data = {
        "model": "deepseek-chat",
        "messages": [
            {"role": "system", "content": SYSTEM_PROMPT},  # 规则放system
            {"role": "user", "content": english_article.strip()}  # 文章放user
        ],
        "temperature": 0.0,  # 0值确保输出稳定
        "max_tokens": 8000,
        "stream": False,
        "response_format": {"type": "json_object"}  # 官方JSON Mode
    }
    ...
    try:
        ...
        logging.info(f"AI原始输出：{ai_raw_output[:500]}...")  # 打印前500字符
...
After studying agriculture, medicine, and fisheries in college, John went back to observing nature and asking questions. Why can certain plants trap harmful bacteria？ Which kinds of fish can eat cancer-causing chemicals? With the right combination of animals and plants, he figured, maybe he could clean up waste the way nature did. He decided to build what he would later call an eco-machine.
    ...
    # 调用API并处理结果
    try:
        result_list = call_deepseek_json_mode(test_article)
        ...
        # 保存到文件
        with open("english_article_result.json", "w", encoding="utf-8") as f:
            json.dump(result_list, f, ensure_ascii=False, indent=2)
        logging.info("结果已保存到 english_article_result.json")
